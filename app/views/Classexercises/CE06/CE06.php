...
foreach ($planets as $planet){
    // pull that stuff into variables for easier reading of the insert statement
    $name = $planet['name'];
    $x = $planet['x'];
    $y = $planet['y'];
    $z = $planet['z'];
    ...
    // Insert Statement
    $insert = "INSERT INTO Planets (PlanetName, PosX, PosY, PosZ, Description) "
        . "VALUES ('$name', '$x', '$y', '$z', '$desc')";
//    echo $insert;
...
for($i = 0; $i < sizeof($planets); $i += 2){
    $name = $planets[$i]['name'];
    $update = "UPDATE Planets SET `Active`= 1 WHERE `PlanetName` = '$name'; ";
...
//search
echo "<br>This is a table of all the plantes<hr>";
$search = "SELECT * FROM Planets ORDER BY PlanetName";
